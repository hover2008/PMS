...
namespace JW.Web.Manage.Areas.PMS.Controllers
{
    ...
    public class DJLBController : BaseAdminController
    {
        #region Fields

        private readonly IDJLBService<DJLBEntity> service;

        #endregion

        #region Ctor

        public DJLBController(IWorkContext workContext,
            IDJLBService<DJLBEntity> service)
            : base(workContext)
        {
        ...
        [HttpGet]
        [HandlerAuthorize("CC-add|CC-edit")]
        public async Task<IActionResult> Form(int id = 0)
        {
            var model = await service.GetModelByIdAsync(id) ?? new DJLBEntity();
            return View(model);
        }
        ...
        [HandlerAuthorize("CC-add|CC-edit")]
        public IActionResult SubmitForm(DJLBEntity model) => Json(service.Save(model));
        ...
        {
            IEnumerable<SelectDJLBEntity> data = await service.GetSelectCanUseListAsync();
            var treeList = new List<SelectModel>();
            foreach (SelectDJLBEntity item in data)
            {
